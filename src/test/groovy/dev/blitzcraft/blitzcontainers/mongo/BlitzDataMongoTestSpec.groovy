...
@BlitzDataMongoTest(tag = '5', isNoTableScan = true)
class BlitzDataMongoTestSpec extends Specification {
    ...
    def 'should save several documents'() {
        given:
        def documents = [new TestDocument('Ilya'), new TestDocument('Tof')]

        when:
        def savedDocuments = repo.saveAll(documents).map { it.name }

        then:
        StepVerifier.create(savedDocuments)
                .recordWith(ArrayList::new)
                .expectNextCount(2)
                .consumeRecordedWith(list -> assertThat(list).containsExactlyInAnyOrder('Ilya', 'Tof'))
                .verifyComplete()
    }
}